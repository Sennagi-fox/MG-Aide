...
local function setattribute(root, view, params, k, v, ids)
  if k == "layout_x" then
   ...
   elseif k == "pages" and type(v) == "table" then --创建页项目
    ...
    local BasePagerAdapter = luajava.bindClass("net.fusion64j.core.ui.adapter.BasePagerAdapter")
   ...
   elseif k == "src" then
    ...
    if not path:find("^/") then
      local runPath = activity.loader.fusionDir .. "/" .. path
      local imagesPath = activity.loader.getImagesDir(path)
      if (File(runPath).exists()) then
        path = runPath
       elseif (File(imagesPath).exists()) then
        path = imagesPath
      end
    end
   ...
   local ColorStateList = luajava.bindClass "android.content.res.ColorStateList"
    view.setBackgroundDrawable(activity.resources.getDrawable(ripple).setColor(ColorStateList(int[0].class{int{v[1]}},int{v[2]})))
